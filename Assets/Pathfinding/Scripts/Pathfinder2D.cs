...
public class Pathfinder2D : MonoBehaviour 
{
    ...
    void Update()
    {
		mapCrTimer -= Time.deltaTime;
		if (mapCrTimer <= 0 && gen == 0) {
    ...
    private void Create2DMap()
    {
        ...
        for (int i = 0; i < height; i++)
        {
            for (int j = 0; j < width; j++)
            {
				...
				//RaycastHit[] hit = Physics.SphereCastAll(new Vector3(x, y, 0), Tilesize / 4, Vector3.forward, dist);
				Collider2D[] hit;
				hit = Physics2D.OverlapAreaAll(new Vector2(x - Tilesize/4,y - Tilesize/4), new Vector2(x + Tilesize/4, y + Tilesize/4));
